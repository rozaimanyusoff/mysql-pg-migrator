...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  const { cfg, tables, include, format, whereClause } = req.body as {
    cfg?: ConnCfg;
    tables?: string[] | 'all';
  ...
  try {
    if (format === 'csv') {
      const result = await exportDatabaseCsv(cfg, tables ?? 'all', whereClause);
      return res.status(200).json(result);
    }
    const result = await exportDatabase(cfg, tables ?? 'all', include ?? 'both', { whereClause });
    return res.status(200).json(result);
  } catch (err: unknown) {

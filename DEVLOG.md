...
- **fix** — Export-Import: round-trip testing of `adms-data` surfaced and fixed 3 bugs in the PG export engine (`src/lib/sql-exporter.ts`)
  ...
  - Bug 2 — custom ENUM types never exported: columns referenced `USER-DEFINED` enum types (e.g. `core.user_type`) by bare `udt_name` but no `CREATE TYPE … AS ENUM` was emitted → import into a fresh schema failed with `type "user_type" does not exist`. Added `pgExportEnumTypes()` which emits idempotent `DO $$ … CREATE TYPE … EXCEPTION WHEN duplicate_object …` blocks before the CREATE TABLEs. Related: schema-qualified default casts (`'x'::core.user_type`) and `nextval('core.seq')` were de-qualified so they resolve to the target schema.
  - Bug 3 (data corruption) — timestamps shifted by server TZ offset: the `pg` driver parsed `timestamp without time zone` into a process-local `Date`, and `pgVal`/`csvVal` emitted `.toISOString()` (UTC), shifting every value by the server's offset (observed −8h on Asia/Kuala_Lumpur). Added per-query `exportTypeParsers` (OIDs 1082/1114/1184 → raw string) on the data-read queries so values are emitted exactly as stored. Fixes both SQL and CSV export.
  - Status: done
- **fix** — Export-Import: schema-level test of `adms-data.stock` (`replace_schema` strategy) surfaced dropped UNIQUE constraints
  - Test: exported `stock` (27 tables, 2,824 rows) and imported into a fresh `stock_test` via `replace_schema`, then re-imported to confirm idempotency. Data was byte-identical both times, but index count was 31 (source) vs 27 (target) — 4 UNIQUE constraints (`stock_card_master_id_store_id_unique`, `trn_prefix_settings_trn_type_unique`, `trn_sequences_prefix_year_month_unique`, `workflow_settings_trn_type_unique`) were missing in the target.
  - Root cause (`src/lib/sql-exporter.ts`, `pgExportSchema`): the index query excluded *every* constraint-backed index (`indexname NOT IN (… table_constraints …)`), removing both PK and UNIQUE indexes, but only PK (inline) and FK (separate ALTER) were re-emitted — UNIQUE constraints were silently dropped, losing uniqueness enforcement on the target.
  - Fix: narrowed the exclusion to `constraint_type = 'PRIMARY KEY'` only, so UNIQUE-constraint indexes flow through the existing index-emit path as `CREATE UNIQUE INDEX IF NOT EXISTS …` — preserves uniqueness enforcement and FK-referenceability. After fix: indexes 31=31, both import and idempotent re-import pass.
  - Status: done
...
- **update** — AI Migration: standardise on Haiku, broaden prompt caching, snappier streaming
  ...
  - Prompt caching: `analyze.ts` and `generate-job.ts` restructured to put static instructions in a cached `system` block (`cache_control: ephemeral`) with only the dynamic schema/db names in the user turn (chat already cached). `explain`/`suggest-columns` left as single-message — their prompts sit below Haiku's ~2048-token cache minimum, so a split wouldn't engage caching.
  - Streaming: SSE routes (`chat`, `analyze`, `generate-job`) now send `Cache-Control: no-cache, no-transform` + `X-Accel-Buffering: no` and call `res.flush?.()` after each event, so deltas render token-by-token instead of buffering in bursts.
  - **Fix (caught in smoke test):** `generate-job` passed `thinking: { type: 'adaptive' }`, which Haiku rejects (400 "adaptive thinking is not supported on this model"). Now gated to Opus only — `...(model.includes('opus') ? { thinking: … } : {})` — so the Haiku default works and an Opus override still gets thinking.
  - Caching caveat: live smoke test confirmed the `analyze`/`generate-job` cached system blocks (rules-only, ~700 tokens) fall **below Haiku's ~2048-token minimum**, so caching does not engage there (`cache_read_input_tokens: 0`). Only `chat` (large dual-schema system) actually caches. The system/user split is kept anyway — it's cleaner and caches on larger models.
  - Note: no MCP / agent / tool-use added — the assistant remains single-shot streaming completions.
  - Verified via direct Haiku smoke test (real API): generate-job → valid JSON, correct type mappings (unsigned bigint→numeric(20,0), tinyint(1) is_*→boolean, enum→text, datetime→timestamp, timestamp→timestamptz) + correct topological order; chat → accurate risk analysis; latencies ~4–5s. `npm run build` + `tsc --noEmit` clean.
  - Status: done
- **implement** — Data Maintenance: unified Copy/Move (relocate) — retires the Sync tab
  - New `src/pages/api/export-import/relocate.ts`: copy or move objects to another live location, generalising sync.ts + replace.ts. Scopes db/schema/table. PG same-database fast path uses `ALTER TABLE … SET SCHEMA` (move) / `CREATE TABLE … (LIKE … INCLUDING ALL) + INSERT` (copy); cross-db/cross-connection path reuses `exportDatabase({ schema })` then imports into the target with `CREATE SCHEMA IF NOT EXISTS` + `SET search_path`; move drops the source only after a successful copy. MySQL: db-scope only (schema/table relocation rejected — no schemas). Cross-type rejected (→ Migration module). FK bypass via `session_replication_role = replica`; transactional with rollback. sync.ts/replace.ts kept (now unused by UI) for future cleanup.
  - UI (`src/pages/export-import.tsx`): retired the Sync tab — removed the tab switcher; the page is a single Import/Export view (`tab` pinned to `'export'`, so the dual-connection/analyse/sync-context-menu branches are now unreachable dead code, not deleted). Added a third persistent per-row action ⇄ (Copy/Move) to DatabasePanel, SchemaPanel, and Panel-4 table rows via `onRowRelocate` (table icon disabled on MySQL with a PostgreSQL-only tooltip). New `RelocateDialogModal` with Copy/Move (equal, no default), a target picker (connection → database → schema, schema as editable datalist for new/existing), and Include + conflict for copy. `runRelocate` posts to `/relocate`, pipes the log into `LogPanel`, and records Saved Jobs (`sync` for copy, `replace` for move). Connection ids typed `number | ''` to match `ConnectionRow.id`.
  - Decisions (confirmed): unify into ⇄ row action + remove Sync tab; offer Copy and Move equally. Plan: `~/.claude/plans/graceful-cuddling-goose.md`.
  - Verified: `npm run build` succeeds (`/api/export-import/relocate` registered); `tsc --noEmit` clean. Manual dev-server check pending.
  - Status: done
- **update** — Data Maintenance: persistent row icons + equal default panel widths
  - `src/pages/export-import.tsx`: per-row import/export icons (DatabasePanel, SchemaPanel, Panel-4 table rows) are now always visible instead of hover-only; db/schema rows get `pr-12` so long names don't collide with the icons. Resizable panels (Database/Schema/Tables) default to equal widths (33/33/34, was 24/18/58); still drag-resizable. Connections (fixed sidebar) and Saved Jobs (collapsible) unchanged.
  - Status: done
- **implement** — Data Maintenance: per-row import/export dialogs (Import/Export view)
  - Reworked `/export-import` so import & export are driven per-row instead of via header toolbar options.
  - UI (`src/pages/export-import.tsx`): collapsed the Export/Import tabs into a single **Import/Export** tab (internal `tab === 'export'`) alongside **Sync**; removed the header `Include`/`Format`/`WHERE` controls and the maintain-view run button. Added hover-revealed export (`Download`) + import (`UploadCloud`) icons to `DatabasePanel` rows, `SchemaPanel` rows, and Panel-4 table rows via new `onRowExport`/`onRowImport` props. New `ExportDialogModal` (Include: schema only / schema + data, Format: SQL/CSV) and `ImportDialogModal` (file drop + mode: Create new / Replace / Insert rows, with statement preview). New `ScopeInfo`/`ImportMode` types and `runScopedExport`/`runScopedImport` handlers send the scope (db/schema/table) + target name. Sync + Replace (dual-connection) kept; their context menus now gate to the Sync tab only. Updated GuidePopover text.
  - Backend export (`src/lib/sql-exporter.ts`, `api/export-import/export.ts`): threaded an optional PG `schema` param through `pgListTablesWithCounts`/`pgExportSchema`/`pgExportData`/`pgExportDataCsv` and `exportDatabase`/`exportDatabaseCsv` (default `public`); index DDL strips the source schema qualifier for portability.
  - Backend import (`api/export-import/import.ts`): added `replace_table` strategy + `schema`/`table` params; schema-scoped `replace_schema` (DROP/CREATE the named schema, not just `public`); create-if-missing + `SET search_path` so create-new/insert land in the chosen schema; MySQL `replace_table` drops the single table first.
  - Decisions (confirmed): keep Sync/Replace alongside; full UI+backend build; export dialog includes format; import target = the clicked row. Plan: `~/.claude/plans/graceful-cuddling-goose.md`.
  - Verified: `npm run build` succeeds (only a pre-existing next.config NFT-trace warning); `tsc --noEmit` clean. Manual dev-server check pending.
  - Status: done

- **implement** — Migration: guided "new schema" banner with Run-now / Save & Schedule actions
  - `src/pages/migration.tsx`: Added a `newTargetTables` memo (included tableMaps whose target table doesn't exist in `tgtTables`) and a guidance banner at the top of the target TABLES panel. When migrating selected source tables into a new/empty target schema, it explains that target tables are auto-created on first run preserving source columns & data types (no per-table mapping needed), and offers two clear next actions: **Run now** (`startMigration`) and **Save & Schedule**.
  - Added `scheduleAfterSave` state + `openSaveAndSchedule` helper. "Save & Schedule" opens the Save dialog (pre-filled `source → schema` name, title/button switch to schedule wording); on success `doSaveJob` redirects to `/scheduler?highlight=<jobId>`, which already auto-opens the Add-Schedule form. Cancelling resets the flag. Imported `Calendar`, `Info` icons.
  - Rationale: the save→schedule path already worked (runner auto-discovers columns when `columns: []`), but was only surfaced via a transient toast. This makes "save config, run later in scheduler" discoverable for the bulk-select-into-new-schema workflow.
  - Status: done
- **update** — Migration: Scenario 2 polish — customizable mapping into a new schema
  - `src/pages/migration.tsx`: Reworded the new-schema guidance notice from "no per-table mapping needed" to make customization discoverable — "each maps 1:1 by default … click any source table to customize its columns, types, or PK → UUID before running." Added a per-row mapping-status badge on source tables: hand-configured maps (`columns.length > 0`) show a violet **custom** badge, or an emerald **set** badge once marked ready; auto (1:1) tables show no badge. Lets users see at a glance which tables they've tuned vs. left on the default 1:1 path. (Scenario 3 — mapping to existing/renamed target tables with PK serial→UUID — deferred; that axis is already covered by the existing column-mapping panel + conversions.)
  - Status: done
- **update** — Migration: new-schema notice gains plain Save + typo guard on schema creation
  - `src/pages/migration.tsx`: Refactored `openSaveAndSchedule` into `openSave(schedule)`; the guidance banner now offers three actions — **Run now**, **Save** (save-only, stays on page), **Save & Schedule** (saves then opens Scheduler) — with a one-line caption clarifying both Save paths produce the same Scheduler-accessible job. Added `confirmNewSchema(raw)`: when the user confirms a brand-new target schema (via the "+" input Enter / "Use" button), it validates the identifier and shows a `showConfirm` dialog warning that the schema will be created on the target DB on first run (typo protection). Existing schemas apply immediately with no prompt.
  - Status: done
- **fix** — Migration: changing target schema now re-points already-selected tables
  - `src/pages/migration.tsx`: Added `changeTgtSchema(next)` and wired it to the schema `<select>` onChange and the new-schema input (Enter + "Use" button). Previously each tableMap froze `target.schema` at creation time (default `public`), so selecting tables first and then switching the schema to e.g. `billings (new)` left the maps — and the actual migration — pointing at `public`. The new handler re-points every map still on the previous default to the newly chosen schema and marks the session dirty. This also fixes the guidance banner showing the wrong schema ("public") because `newTargetSchema` derives from the maps, which now follow the selection.
  - Status: done
